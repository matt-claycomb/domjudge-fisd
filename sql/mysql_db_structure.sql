...
CREATE TABLE `problem` (
  ...
  `filename` VARCHAR(255) DEFAULT NULL COMMENT 'File name (if any) for problem input',
  PRIMARY KEY  (`probid`)
...
CREATE TABLE `submission` (
  ...
  `rejudgingid` int(4) unsigned DEFAULT NULL COMMENT 'Rejudging ID (if rejudge)',
  `expected_results` varchar(255) DEFAULT '["CORRECT"]' COMMENT 'JSON encoded list of expected results - used to validate jury submissions',
